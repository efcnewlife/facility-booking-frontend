...
**Booking cart**:
The right-hand panel on the Timetable. For One-time it lists confirmed Booking lines after Confirm Booking Time; each line shows a room thumbnail, name, that line's time, line subtotal, Remove, and Edit. For Repeated it lists selected rooms under one shared time window; rows show the shared time and Remove but no per-room subtotal or independent Edit. Review Booking sits at the top of either mode. Removing a One-time line restores ADD on that Timetable block. A Repeated cart is cleared as a whole when its shared time is replaced. The One-time cart persists in the browser's localStorage keyed to the search date and ministry, so a page refresh does not lose it; a stored cart for a different date or ministry is treated as empty rather than restored. The Repeated cart is not written to that store, so its rooms and shared time do not mix with independently timed One-time lines.
_Avoid_: cart on Booking Details as the primary picker, Review Booking only for Multiple, independently timed Repeated rooms, a Repeated per-room subtotal, assuming three is still the cap, assuming a refresh clears the One-time cart, restoring Repeated rooms from the One-time cart store
