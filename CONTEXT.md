...
**Repeated**:
A booking frequency: the same weekly interval on a repeating schedule. Member copy uses this word, not Recurring. Choosing Repeated in Start booking collects First occurrence / weekday and optional Start Time / End Time, then continues to `/rooms` in Repeated mode. These optional times are search seed only; they do not lock the Series. Selecting First occurrence derives the weekday; selecting a weekday requires the member to choose a matching First occurrence. The Recurring schedule card selects Last occurrence. The first room interval confirmed into the Repeated Booking cart becomes the shared time window; another interval requires explicit confirmation to replace that shared time and clears every selected room. A complete proposal previews the full Recurring Booking Series without reserving rooms, then Review always precedes create — including when the preview is conflict-free — and a successful create stays Pending-payment. Review distinguishes occupancy, Blackout, Weekly Rental Booking quota, and Ministry conflicts from that server result. The member may exclude only dates the current preview reported as conflicts; Blackout and protected Ministry conflicts cannot be overridden. Revising the shared schedule or selected rooms returns the member to `/rooms` for a fresh preview before another create.
