...
**Search Bar**:
The editable summary on the Timetable: Ministry (when shown), Repetition, Date, plus Update search. Controls are one size step larger than before. It does not show start time, end time, or room count. The Ministry field is hidden unless the person is a Ministry member of at least one active ministry they can book for. When the field shows and the search is Non-ministry, it shows None and they may attach such a ministry. Holding the Owner position does not by itself show this field. Update search applies date and ministry changes. In Repeated mode the same bar uses Start Date, End Date, and Repeat on instead of Date: Start Date is First occurrence and the weekday source of truth, Repeat on can be chosen first and then requires a matching Start Date, End Date is Last occurrence, and Update search applies them together with ministry. The weekly occurrence count is not shown here; the Booking cart summary already states it. There is no separate Recurring schedule card.
_Avoid_: Start Time and End Time on the bar, # of rooms, Single/Multiple, Room shortcut, a read-only recap, showing Ministry for Owner position alone, treating pending-only applicants as able to attach a ministry, treating Repeated selection as independently timed room lines, a second Starts on date next to Date, duplicating the weekly occurrence count beside End Date when the cart already states it

**Booking cart**:
The right-hand panel on the Timetable. Its summary card collects the required Booking Title, shows one server-authoritative Estimated Total, and states the number of selected rooms. When the proposal belongs to a Ministry, it also shows that Ministry's name. In Repeated mode it also states the shared usage time and the complete date-range recurrence count, before later conflict exclusions. Below it, One-time lists confirmed Booking lines after Confirm Booking Time; each line shows a room thumbnail, name, that line's time, line subtotal, Remove, and Edit. Repeated lists selected rooms under one shared time window; rows show the shared time and Remove but no per-room subtotal or independent Edit. Review & Confirm stays fixed at the bottom of either mode, while a long room list scrolls. It creates a Draft for a new proposal or updates its existing Draft before opening Booking Details. It remains disabled until that Draft-ready proposal has a valid Title and a current server price preview. Returning from Booking Details restores the Draft's Title, rooms, and schedule to this cart. A Title-only change neither clears rooms nor recomputes the price; a schedule, room, or Ministry change requires a new server price preview. Before the first Review & Confirm, a One-time search change clears its rooms and old price but retains its Title. Removing a One-time line restores ADD on that Timetable block. A Repeated cart is cleared as a whole when its shared time is replaced. The One-time cart, including its Title, persists in the browser's localStorage keyed to the search date and ministry, so a page refresh does not lose it; a stored cart for a different date or ministry is treated as empty rather than restored. The Repeated cart is not written to that store, so its rooms, shared time, and Title do not survive a refresh or mix with independently timed One-time lines.
_Avoid_: cart on Booking Details as the primary picker, Review Booking only for Multiple, independently timed Repeated rooms, a Repeated per-room subtotal, calculating the total only in the browser, continuing while a current price preview is unavailable, assuming three is still the cap, assuming a refresh clears the One-time cart, restoring Repeated rooms from the One-time cart store
