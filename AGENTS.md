...
| Facility | `/facility/rooms/availability`, `/facility/bookings`, `/facility/bookings/mine`, `/facility/booking-series` |

Defined but unused by UI today: `MINISTRY.MINISTRY_TYPES`, `MINISTRY.TARGET_AUDIENCES`.
...
| `facilityService` | `getAvailability`, `createBooking`, `listMyBookings`, Series get/cancel   |
...
Typical progression: ministry yes/no → (ministry name if Yes) → One-time vs Repeated. How often always shows that Repeated is only open during specific periods (December 1 / June 1), including the next opening date when closed, without waiting for Repeated to be selected. One-time continues through When (date plus optional start/end) to `/rooms`. Repeated continues through First occurrence / weekday plus optional Start Time / End Time (`recurring_when`) to the same `/rooms` route with `frequency=repeated` only when the Recurring Booking availability window is open; otherwise Continue stays disabled. Those times seed the timetable search only; they do not lock the Series. Create ministry is a modal on ministry name. The current step is mirrored into the URL with `{ replace: true }`.

### 3. Results — `src/pages/rooms/RoomFilterPage.tsx`

Reads Start booking output via `parseRoomsSearchQuery`. One-time: missing or invalid `date` redirects to Home. Repeated: missing `date` stays on `/rooms` until Start Date is set and does not require a time seed; a complete optional `start`/`end` pair is search highlight only. If the Recurring Booking availability window is closed, Repeated `/rooms` redirects to Start booking frequency. Search Bar collects Start Date, End Date, and Repeat on (weekday); Update search applies `date`, `lastDate`, and `weekday`. Legacy `space` / `room` URL params are ignored on entry. Loads via `facilityService.getAvailability(date, ministryId)` for the First occurrence date. One-time can create a booking from the cart. Repeated uses the same pin, ADD, Confirm Booking Time, and Booking cart interaction; Confirm Booking Time requires End Date before Confirm; the first confirmed interval locks shared time, and a different interval asks to replace it.
...
| `my-bookings/MyBookingsPage.tsx`            | Upcoming/past live list; Series grouped              |
| `my-bookings/RecurringSeriesDetailPage.tsx` | Series occurrences, payment hold, scoped cancel      |
...
| Assume profile / contact are live APIs                       | Still mock or stub                       |
...
| `src/api/services/facilityService.ts`          | Availability, create, My Bookings, Series |
...
| My Bookings grouping / cancel | `MyBookingsPage` + `RecurringSeriesDetailPage` + `src/utils/myBookings.ts` |
| Profile still fake            | Replace `data/mockProfile.ts` imports                                      |
